...
R accesses files the same way you would. So for example, you can set your working directory for a particular R session using the set-working-directory command (setwd). When using RStudio (we'll get to this later), however, the working directory is automatically set when you open a project.
...
```{r haplo_freq, echo=T}
# load the libraries - install these if you don't already have them
library(ggplot2)
library(dplyr)
library(tidyr)
...
#count is a function in dplyr to create a frequency table of our data
...
bar2<-bar1 + theme(axis.title.x = element_text(face="bold", 
  ...
  xlab("Nesting Stock")+ylab("Estimated Contribution") + 
  coord_cartesian(ylim=c(-0.01,0.78))
print(bar2)
