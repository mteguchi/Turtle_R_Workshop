...
![alt text](images/home_directory.png)

In a Mac OS, it may look like this in Lisa's computer:

![alt text](images/Mac_path_screenshot.pdf)

When you look at the complete path, this looks like this: "C:\\users\\tomo\.eguchi\\". And within that directory, I have several directories that contain different things, like music, pictures, documents, etc. 

I suggest you create an R directory somewhere in this home directory. In my case, I created one under "My Documents", like so:

![alt text](images/R_directory.png)
...
![alt text](images/CMR_Australia_dir.png)

As you can see, I haven't done good job of keeping up with organizing everything... But you get the picture. 

R accesses files the same way you would. So for example, you can set your working directory for a particular R session using the set-working-directory command (setwd):

```{r setwd, echo=TRUE}
#setwd("~/My Documents/R/CMR_Australia")
...
Did you accidentally put the data that you wanted in another folder? The most common errors importing data can be from simply being in a folder above where you the file actually is, etc. (or misspelling-the list.files()-then allows you to copy and paste to avoid that)
...
When using RStudio (we'll get to this later), however, the working directory is automatically set when you open a project. 
...
#Packages
One of the strengths of R is the ability to utilize 'packages'. A package is an extra set of functions, code, and/or data that expands the capability of R, often written by scientists who needed some functionality that didn't yet exist within R. 
As of May 2016, there are 8,349 packages available for download that span a wide range of topics. These include: genomics, mapping, time-series analysis, plotting, and much more. 
The full list of packages available can be found here. 
https://cran.r-project.org/web/packages/
Packages must be downloaded once using the following code. Once the package is installed, this command also updates the package. Let's download ggplot2, a graphing package.

A package needs to be installed on your computer, then loaded when you use it:

install.packages("ggplot2") 
library(ggplot2)

It would be best to load the packages that you'll need in the current session to keep R efficient - all the files in packages are stored in the computer's memory.
...
There are two primary components to the ggplot command. The first is to define the data and the visual aesthetic. First we define a dataframe, then provide what kind of plots you want to make. Other components define details of plots, such as font size, background color, axis labels, etc. 
...
Can we rotate the tick marks a little? To do this in R plots, we need to clear the axis tick labels and add in text (see http://www.ats.ucla.edu/stat/r/faq/angled_labels.htm). In ggplot, things are a bit quicker:
...
For another example, we'll look at how to plot two series (e.g., temmperature and pH) that share a variable (date/time). 
...
Alternatively, we want to plot these in one plot:
``` {r two_series, echo=T}
# Turns out this is not that easy! 
# Do a bit of tricks to the second plot;
plot2 <- ggplot(data = df.temp_pH, 
                aes(x = Date, y = pH)) +
  geom_line(color = "red") + 
  xlab("Date") + 
...
It appears that the second y axis cannot be labeled at the moment using ggplot2... the authors of the package apparently disagree philosophically with having two y axes. I can understand that. 
...
*You may install these if you don't already have them
```{r haplo_load, echo=T}
library(ggplot2)
library(dplyr)
library(tidyr)
...
data<-data.frame(location = rep(c('Mainland MU', 
                   'Yakushima MU',
                   'Ryukyu MU',
                   'Muroto bycatch'), each=10), #repeat command, create a list of site names, each 10 times 
...
head(data)#first five rows of dataframe
```

This data is by individual, typically what we'd have in a raw data file. But we may want to instead have summarized data for different graphs or analyses. We can use the 'count' function in dplyr to create a frequency table of our data
